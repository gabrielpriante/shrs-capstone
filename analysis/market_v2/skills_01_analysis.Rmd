---
title: "SHRS Market Analysis — Skills & Education Profiles (v2.1)"
...
**v2.1 Change:** 9 programs across all 6 SHRS departments, each with a unique
SOC code. SS (Exercise Physiologists) restored; OT-CScD duplicate removed.
...
```{r bls-heatmap, fig.height=9, fig.width=11}
bls_skills |>
...
```{r bls-radar, fig.height=14, fig.width=10}
bls_skills |>
...
Which skills most differentiate each program from the average of the other
programs?
...
```{r onet-top10, fig.height=14, fig.width=10}
onet_skills |>
  group_by(shrs_program) |>
...
```{r onet-heatmap, fig.height=14, fig.width=11}
onet_skills |>
...
```{r cluster-heatmap, fig.height=6}
onet_clustered |>
...
```{r positioning, fig.height=7, fig.width=9}
...
With 9 programs now in scope, we can do richer cross-program
comparisons to identify shared skill foundations and unique differentiators.

## 5.1 Programs Clustered by Skill Similarity

```{r skill-distance, fig.height=7}
