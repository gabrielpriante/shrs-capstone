---
title: "SHRS Market Analysis — Industry Employment by Sector (2023 Vintage)"
subtitle: "BLS Employment Projections Table 2.1 · 2023–2033 Cycle"
...
DATA_PATH <- "/Users/gpps/Documents/SHRS_Analysis/market_analysis/v3data/projections_csv/industry_csv/2023/2023_industry - Table 2.1.csv"
...
Table 2.1 (2023 vintage) uses the same 14-column layout as the 2024 vintage but
references a different time window: 2013 (historical), 2023 (base), 2033 (projected).
...
| 2 | 2022 NAICS code |
| 3 | Employment, 2013 (thousands) |
| 4 | Employment, 2023 (thousands) |
| 5 | Employment, 2033 (thousands) |
| 6 | Employment change, numeric, 2013–23 |
| 7 | Employment change, numeric, 2023–33 |
| 8 | Employment change, percent, 2013–23 |
| 9 | Employment change, percent, 2023–33 |
| 10 | Percent distribution, 2013 |
| 11 | Percent distribution, 2023 |
| 12 | Percent distribution, 2033 |
| 13 | Compound annual rate of change, 2013–23 |
| 14 | Compound annual rate of change, 2023–33 |

**Key structural note:** The 2023 vintage includes a "Nonagriculture" row and
groups some sectors differently (e.g., "Leisure and hospitality" instead of
separate Arts/Accommodation rows, "Financial activities" instead of separate
Finance/Real estate). This means the sector count differs from 2024.
...
sectors <- df %>%
  filter(!is.na(emp_base),
         !str_detect(industry_sector,
           "^Total|^Self-employed|^Total wage|^Nonagriculture$|^Goods-producing|^Service-providing")) %>%
...
```{r stored-outputs}
ind_t21_2023_vintage       <- VINTAGE
ind_t21_2023_cycle         <- CYCLE
ind_t21_2023_total_emp     <- total_emp_base
ind_t21_2023_total_proj    <- total_emp_proj
ind_t21_2023_total_pct     <- total_pct_proj
ind_t21_2023_total_cagr    <- total_cagr_proj
ind_t21_2023_hc_emp        <- hc_emp_base
ind_t21_2023_hc_proj       <- hc_emp_proj
ind_t21_2023_hc_pct        <- hc_pct_proj
ind_t21_2023_hc_cagr       <- hc_cagr_proj
ind_t21_2023_hc_share_base <- hc_pct_dist_base
ind_t21_2023_hc_share_proj <- hc_pct_dist_proj
ind_t21_2023_growth_ratio  <- growth_ratio
ind_t21_2023_scorecard     <- scorecard_df
ind_t21_2023_composite     <- composite_score
ind_t21_2023_signal        <- signal
ind_t21_2023_sectors       <- rankings
```

```{r output-registry}
output_registry <- tribble(
  ~variable,                         ~description,
  "ind_t21_2023_vintage",            "Projection vintage year (2023)",
  "ind_t21_2023_cycle",              "Projection cycle label (2023-2033)",
  "ind_t21_2023_total_emp",          "Total employment, base year (thousands)",
  "ind_t21_2023_total_proj",         "Total employment, projected (thousands)",
  "ind_t21_2023_total_pct",          "Total employment projected growth %",
  "ind_t21_2023_total_cagr",         "Total employment CAGR %",
  "ind_t21_2023_hc_emp",             "Healthcare & social assistance employment, base (thousands)",
  "ind_t21_2023_hc_proj",            "Healthcare & social assistance employment, projected (thousands)",
  "ind_t21_2023_hc_pct",             "Healthcare sector projected growth %",
  "ind_t21_2023_hc_cagr",            "Healthcare sector CAGR %",
  "ind_t21_2023_hc_share_base",      "Healthcare share of total employment, base year %",
  "ind_t21_2023_hc_share_proj",      "Healthcare share of total employment, projected %",
  "ind_t21_2023_growth_ratio",       "Healthcare growth / all-industry growth ratio",
  "ind_t21_2023_scorecard",          "Full scorecard dataframe (4 dimensions + composite)",
  "ind_t21_2023_composite",          "Composite scorecard score (0-100)",
  "ind_t21_2023_signal",             "Overall signal: Green/Yellow/Red",
  "ind_t21_2023_sectors",            "Full sector-level dataframe with rankings"
)

output_registry %>%
  kable(caption = "Output Variable Registry — industry_t21_2023.Rmd") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
