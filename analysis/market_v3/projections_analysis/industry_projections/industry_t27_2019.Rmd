---
title: "SHRS Market Analysis — Detailed Industry Employment (2019 Vintage)"
subtitle: "BLS Employment Projections Table 2.7 · 2019–2029 Cycle"
...
DATA_PATH <- "/Users/gpps/Documents/SHRS_Analysis/market_analysis/v3data/projections_csv/industry_csv/2019/2019_industry_2019 - Table 2.7.csv"
...
**CRITICAL:** The 2019 vintage uses **Table 2.7** (not 2.11) for detailed
industry data. The table name changed in later vintages but the content is
equivalent. This file has a **3-row multi-row header**:

- Row 1: Top-level category groups ("Industry, 2017 NAICS", "Employment", "Output")
- Row 2: Sub-category groups ("Thousands of Jobs", "Change", "CAGR", "Billions...")
- Row 3: Year labels ("2009", "2019", "2029", etc.)
- Row 4+: Data

We skip all 3 header rows. The underlying data structure is the same 14 columns
as 2020/2021/2022. Extra whitespace is present in many values. This is the
**last pre-COVID** projection cycle.

```{r load-data}
raw <- read_csv(DATA_PATH, col_types = cols(.default = "c"), col_names = FALSE)

# Skip the 3-row header
raw <- raw[-(1:3), ]
...
raw <- raw %>%
  ...
  mutate(naics_code = str_trim(naics_code),
         naics_code = if_else(naics_code == "NA" | naics_code == "" | is.na(naics_code),
                              NA_character_, naics_code))
...
cat("Loaded", nrow(df), "rows for", CYCLE, " (last pre-COVID cycle)\n")
```

```{r preview}
df %>% select(industry_title,naics_code,emp_base,emp_proj,emp_pct_proj,cagr_proj) %>%
  head(10) %>% kable(digits=1,format.args=list(big.mark=","),caption="Table 2.7 Preview") %>%
...
shrs_ind %>%
  ...
  kable(digits=1, format.args=list(big.mark=","),
        caption=paste0("SHRS Industries (", CYCLE, ") — Pre-COVID baseline")) %>%
  kable_styling(bootstrap_options=c("striped","hover"), full_width=FALSE)
```
...
shrs_ind %>% filter(!is.na(emp_pct_proj)) %>%
  ...
  labs(title=paste0("SHRS Industry Growth (", CYCLE, ")"),
       subtitle="PRE-COVID baseline | * = 4-digit NAICS proxy",
       x=NULL, y="Growth (%)") +
...
momentum %>% select(short_name,emp_pct_hist,emp_pct_proj) %>%
  ...
  mutate(period=if_else(period=="emp_pct_hist","2009-2019","2019-2029")) %>%
  ggplot(aes(x=short_name,y=growth,fill=period)) +
  geom_col(position="dodge",width=0.6) + coord_flip() +
  scale_fill_manual(values=c("2009-2019"="#abd9e9","2019-2029"="#2c7bb6")) +
  labs(title="Historical vs Projected Growth (Pre-COVID)",x=NULL,y="Growth (%)",fill=NULL) +
  theme_minimal(base_size=12) + theme(legend.position="bottom")
```
...
program_ind %>% arrange(desc(composite_score)) %>%
  ...
  kable(digits=1,format.args=list(big.mark=","),
        caption=paste0("Program Scorecard (",CYCLE,") — Pre-COVID")) %>%
  kable_styling(bootstrap_options=c("striped","hover"),full_width=FALSE) %>%
...
```{r stored-outputs}
ind_t27_2019_vintage <- VINTAGE; ind_t27_2019_cycle <- CYCLE
ind_t27_2019_total_pct <- total_pct; ind_t27_2019_total_cagr <- total_cagr
ind_t27_2019_hc_pct <- hc_pct; ind_t27_2019_hc_cagr <- hc_cagr; ind_t27_2019_hc_emp <- hc_emp_base
ind_t27_2019_shrs_ind <- shrs_ind; ind_t27_2019_ind_6213 <- ind_6213
ind_t27_2019_ind_6219 <- ind_6219; ind_t27_2019_program_scores <- program_ind
ind_t27_2019_momentum <- momentum; ind_t27_2019_full_df <- df
ind_t27_2019_pre_covid_note <- "Last pre-COVID projection cycle — clean baseline"
ind_t27_2019_table_note <- "Uses Table 2.7 (renamed to 2.11 in later vintages)"
ind_t27_2019_naics_note <- "4-digit NAICS only; 621340/621990 unavailable"
```

```{r registry}
tribble(~variable,~description,
  "ind_t27_2019_vintage","Vintage (2019)","ind_t27_2019_cycle","Cycle (2019-2029)",
  "ind_t27_2019_total_pct","Total growth %","ind_t27_2019_hc_pct","HC growth %",
  "ind_t27_2019_shrs_ind","SHRS industries df","ind_t27_2019_ind_6213","NAICS 6213 (proxy 621340)",
  "ind_t27_2019_ind_6219","NAICS 6219 (proxy 621990)","ind_t27_2019_program_scores","Scorecard",
  "ind_t27_2019_momentum","Momentum analysis","ind_t27_2019_pre_covid_note","Pre-COVID note",
  "ind_t27_2019_table_note","Table 2.7 (=2.11 in later vintages)"
) %>% kable(caption="Output Registry — industry_t27_2019.Rmd") %>%
  kable_styling(bootstrap_options=c("striped","hover"),full_width=FALSE)
```
