---
title: "SHRS Market Analysis — Detailed Industry Employment (2021 Vintage)"
subtitle: "BLS Employment Projections Table 2.11 · 2021–2031 Cycle"
...
DATA_PATH <- "/Users/gpps/Documents/SHRS_Analysis/market_analysis/v3data/projections_csv/industry_csv/2021/2021_industry_2021 - Table 2.11.csv"
VINTAGE <- 2021; CYCLE <- "2021-2031"
...
Same 14-column structure as 2022: industry + NAICS in two columns, historical +
projected employment, CAGRs, output data. Uses **2017 NAICS** classification.
Time window: 2011 (hist), 2021 (base), 2031 (proj). Blank separator rows present.
...
cat("Found", nrow(shrs_ind), "target industries\n")
```
...
shrs_ind %>%
  ...
  kable(digits=1, format.args=list(big.mark=","), caption=paste0("SHRS Industries (", CYCLE, ")")) %>%
...
shrs_ind %>% filter(!is.na(emp_pct_proj)) %>%
  ...
  labs(title=paste0("SHRS Industry Growth (", CYCLE, ")"),
       subtitle="* = 4-digit NAICS proxy", x=NULL, y="Growth (%)") +
...
cat("NAICS 6213: ", ind_6213$emp_pct_proj, "% growth | CAGR ", ind_6213$cagr_proj, "%\n")
```

```{r ind-6219}
ind_6219 <- df %>% filter(naics_code == "6219")
cat("NAICS 6219: ", ind_6219$emp_pct_proj, "% growth | CAGR ", ind_6219$cagr_proj, "%\n")
```
...
momentum <- shrs_ind %>%
  ...
  mutate(short_name = case_when(
    ...
    direction = if_else(mom >= 0, "Accelerating", "Decelerating"))

momentum %>% select(short_name,emp_pct_hist,emp_pct_proj) %>%
  pivot_longer(-short_name,names_to="period",values_to="growth") %>%
  mutate(period=if_else(period=="emp_pct_hist","2011-2021","2021-2031")) %>%
  ggplot(aes(x=short_name,y=growth,fill=period)) +
  geom_col(position="dodge",width=0.6) + coord_flip() +
  scale_fill_manual(values=c("2011-2021"="#abd9e9","2021-2031"="#2c7bb6")) +
  labs(title="Historical vs Projected Growth",x=NULL,y="Growth (%)",fill=NULL) +
  theme_minimal(base_size=12) + theme(legend.position="bottom")
```
...
```{r stored-outputs}
ind_t211_2021_vintage <- VINTAGE; ind_t211_2021_cycle <- CYCLE
ind_t211_2021_total_pct <- total_pct; ind_t211_2021_total_cagr <- total_cagr
ind_t211_2021_hc_pct <- hc_pct; ind_t211_2021_hc_cagr <- hc_cagr; ind_t211_2021_hc_emp <- hc_emp_base
ind_t211_2021_shrs_ind <- shrs_ind; ind_t211_2021_ind_6213 <- ind_6213
ind_t211_2021_ind_6219 <- ind_6219; ind_t211_2021_program_scores <- program_ind
ind_t211_2021_momentum <- momentum; ind_t211_2021_full_df <- df
ind_t211_2021_naics_note <- "4-digit NAICS only; 621340/621990 unavailable"
```

```{r registry}
tribble(~variable,~description,
  "ind_t211_2021_vintage","Vintage (2021)","ind_t211_2021_cycle","Cycle (2021-2031)",
  "ind_t211_2021_total_pct","Total growth %","ind_t211_2021_hc_pct","HC growth %",
  "ind_t211_2021_shrs_ind","SHRS industries df","ind_t211_2021_ind_6213","NAICS 6213 (proxy 621340)",
  "ind_t211_2021_ind_6219","NAICS 6219 (proxy 621990)","ind_t211_2021_program_scores","Program scorecard",
  "ind_t211_2021_momentum","Momentum analysis","ind_t211_2021_full_df","Full df"
) %>% kable(caption="Output Registry — industry_t211_2021.Rmd") %>%
  kable_styling(bootstrap_options=c("striped","hover"),full_width=FALSE)
```
