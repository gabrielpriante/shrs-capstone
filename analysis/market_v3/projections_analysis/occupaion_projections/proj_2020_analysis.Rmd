---
title: "SHRS Market Analysis — BLS Projections 2020–2030"
...
In the 2020 cycle, the HIM field is reported under the **combined** code
29-2098 ("Medical dosimetrists, medical records specialists, and health
technologists and technicians, all other"). This is a broader bucket than
the 29-9021 code used starting in 2021, so HIM estimates from this cycle
should be interpreted with caution — they include non-HIM occupations.
...
soc_crosswalk <- tribble(
  ...
  "HIM",         "HIM",       "29-2098",  "Medical Dosimetrists, Medical Records Specialists, and Health Technologists (combined)",
...
PROJ_CSV_PATH <- file.path(path.expand("~"), "Documents", "SHRS_Analysis",
                            ...
                            "occupation_csv", "occupation_2020.csv")
...
# COVID-19 context flag
cat("\n⚠ NOTE: The 2020 base year reflects COVID-19 pandemic labor market\n")
cat("  disruptions. Growth rates from this cycle may appear elevated due to\n")
cat("  recovery effects from a depressed 2020 baseline.\n")
```
...
proj |>
  ...
  labs(
    ...
    subtitle = "BLS National Employment Matrix | ⚠ 2020 baseline reflects COVID-19 effects",
...
proj |>
  ...
  labs(title = paste0("Separation Rates (", CYCLE, ")"),
       subtitle = "Dashed = all-occupations benchmark",
...
proj |>
  mutate(market_size = case_when(
    emp_base >= 200 ~ "Large (≥200K)", emp_base >= 50 ~ "Mid-size (50–200K)",
    emp_base >= 20 ~ "Small (20–50K)", TRUE ~ "Niche (<20K)"),
    market_size = factor(market_size, levels = c("Large (≥200K)",
      "Mid-size (50–200K)", "Small (20–50K)", "Niche (<20K)"))) |>
  ...
  geom_hline(yintercept = all_occ_growth_pct, linetype = "dashed",
             color = "gray40") +
  scale_color_manual(values = c("Large (≥200K)" = "#1a9850",
    "Mid-size (50–200K)" = "#4393c3", "Small (20–50K)" = "#fee08b",
    "Niche (<20K)" = "#d73027")) +
...
proj |>
  ...
  scale_fill_manual(values = c("High" = "#1a9850", "Normal" = "#4393c3"),
                     guide = "none") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
  labs(title = "Openings Intensity: Annual Openings per 1,000 Workers",
       subtitle = paste0(CYCLE, " cycle"),
...
scorecard |>
  ...
  labs(title = paste0("Market Demand Scorecard (", CYCLE, ")"),
       subtitle = "Min-max scaled 0–100 across 9 SHRS programs",
       x = NULL, y = NULL) +
  theme_minimal(base_size = 13) +
...
cat("Growth Rate (%): Mean:", round(mean(proj$emp_change_pct), 2),
    "| Median:", round(median(proj$emp_change_pct), 2),
    "| SD:", round(sd(proj$emp_change_pct), 2), "\n")
cat("  Range:", min(proj$emp_change_pct), "to", max(proj$emp_change_pct), "\n\n")
cat("Annual Openings (K): Mean:", round(mean(proj$annual_openings), 2),
    "| Total:", round(sum(proj$annual_openings), 2), "\n\n")
cat("Sep Rate (%): Mean:", round(mean(proj$total_sep_rate), 2),
    "| Range:", min(proj$total_sep_rate), "to", max(proj$total_sep_rate), "\n\n")
cat("Programs above benchmark:", sum(proj$emp_change_pct >= all_occ_growth_pct),
    "of 9\n")
```
...
cor_growth_sep <- cor(proj$emp_change_pct, proj$total_sep_rate,
                       use = "complete.obs")
cat("Correlation (Growth ↔ Openings):", round(cor_growth_openings, 3), "\n")
...
```{r stored-outputs}
proj_2020_data       <- proj
scorecard_2020_data  <- scorecard
benchmark_2020 <- list(cycle = CYCLE, all_occ_growth = all_occ_growth_pct,
                        all_occ_sep_rate = all_occ_sep_rate,
                        all_occ_openings = all_occ_openings)
summary_stats_2020 <- list(
...
program_signals_2020 <- scorecard |>
...
cat("  proj_2020_data         —", nrow(proj_2020_data), "rows\n")
cat("  scorecard_2020_data    —", nrow(scorecard_2020_data), "rows\n")
cat("  program_signals_2020   —", nrow(program_signals_2020), "rows\n")
cat("  benchmark_2020         — growth:", benchmark_2020$all_occ_growth, "%\n")
cat("  summary_stats_2020     — avg:", summary_stats_2020$growth_mean, "%\n")
cat("\n⚠ HIM note: SOC 29-2098 (combined code) used in this cycle.\n")
```
...
**Source:** BLS National Employment Matrix, 2020–2030 (`occupation_2020.csv`)

**COVID-19 caveat:** The 2020 base year was heavily affected by pandemic-related
employment losses. Growth rates from this cycle are inflated relative to
pre/post-pandemic cycles due to the depressed baseline.

**HIM SOC note:** Code 29-2098 includes medical dosimetrists, medical records
specialists, and other health technologists — broader than 29-9021 used in
2021+ cycles.

**Scorecard:** 4 dimensions, 0–100 min-max, composite = equal-weighted average.
