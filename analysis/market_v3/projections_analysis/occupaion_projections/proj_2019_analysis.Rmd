---
title: "SHRS Market Analysis — BLS Projections 2019–2029"
...
The 2019 cycle uses SOC 29-2098 (combined code) for HIM, same as 2020.
...
PROJ_CSV_PATH <- file.path(path.expand("~"), "Documents", "SHRS_Analysis",
                            ...
                            "occupation_csv", "occupation_2019.csv")
...
proj |>
  ...
  labs(
    ...
    subtitle = "BLS National Employment Matrix | Last pre-COVID baseline",
...
proj |>
  mutate(
    openings_signal = case_when(
      annual_openings >= 10 ~ "🟢 High Volume", annual_openings >= 3 ~ "🟢 Solid",
      annual_openings >= 1 ~ "🟡 Moderate", TRUE ~ "🔴 Limited"),
    replacement_signal = case_when(
      total_sep_rate >= 6 ~ "🟢 High Replacement",
      total_sep_rate >= 4 ~ "🟡 Moderate", TRUE ~ "🔴 Low")
  ) |>
...
proj |>
  ...
  scale_fill_manual(values = c("High" = "#1a9850", "Normal" = "#4393c3"),
                     guide = "none") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
  labs(title = "Openings Intensity (per 1,000 workers)",
       subtitle = CYCLE, x = NULL, y = "Openings per 1,000 workers") +
  theme_minimal(base_size = 13)
```

---

# 4 — Scorecard (0–100 Scale)
...
cor_growth_sep <- cor(proj$emp_change_pct, proj$total_sep_rate,
                       use = "complete.obs")
cat("Corr Growth↔Openings:", round(cor_growth_openings, 3), "\n")
cat("Corr Growth↔Sep:", round(cor_growth_sep, 3), "\n")
```
...
```{r stored-outputs}
proj_2019_data       <- proj
scorecard_2019_data  <- scorecard
benchmark_2019 <- list(cycle = CYCLE, all_occ_growth = all_occ_growth_pct,
                        all_occ_sep_rate = all_occ_sep_rate,
                        all_occ_openings = all_occ_openings)
summary_stats_2019 <- list(
...
program_signals_2019 <- scorecard |>
...
cat("  proj_2019_data, scorecard_2019_data, benchmark_2019\n")
cat("  summary_stats_2019, program_signals_2019, soc_crosswalk_9\n")
cat("  HIM SOC: 29-2098 (combined code)\n")
```
...
**Source:** BLS National Employment Matrix, 2019–2029 (`occupation_2019.csv`)

**Context:** This is the last pre-COVID projection cycle — base year employment
reflects normal labor market conditions before pandemic disruptions. This makes
it a valuable benchmark for assessing whether post-pandemic projections
represent genuine structural shifts or recovery effects.

**HIM SOC:** 29-2098 (combined). **Scorecard:** 0–100, 4 dimensions, Green/Yellow/Red.
