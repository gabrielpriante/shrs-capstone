---
title: "SHRS Education Analysis — 2019 Workforce Attainment (Table 5.3)"
...
soc_xwalk <- tribble(
  ...
  "SLP", "CSD", "29-1127", "AuD", "CSD", "29-1181",
  "HIM", "HIM", "29-9021", "OTD", "RST", "29-1122",
  "DPT", "RST", "29-1123", "PAS", "PAS", "29-1071",
  "AT",  "SMN", "29-9091", "DN",  "SMN", "29-1031",
  "SS",  "SMN", "29-1128"
)
...
shrs_long <- shrs_wide |>
  pivot_longer(cols = c(less_than_hs, hs_diploma, some_college, associates,
...
shrs_long |>
  ...
  labs(title = paste0("Workforce Attainment (", VINTAGE, ")"),
       subtitle = "BLS Table 5.3 — HIM not available", x = NULL, y = "% of Workers") +
...
```{r dist-table}
shrs_wide |>
  select(Program=shrs_program, Dept=shrs_dept, `<HS`=less_than_hs, HS=hs_diploma,
         `Some Col`=some_college, Assoc=associates, Bach=bachelors, Master=masters, Doct=doctoral) |>
...
grad_share <- grad_share |>
  mutate(benchmark = all_occ_grad, ratio_to_benchmark = round(grad_pct / all_occ_grad, 2))
```
...
grad_share |>
  ...
  annotate("text", x=1.5, y=all_occ_grad+2, label=paste0("All occ: ",all_occ_grad,"%"),
  ...
  labs(title=paste0("Graduate Concentration (",VINTAGE,")"), x=NULL, y="% Grad Degree") +
...
```{r store}
t53_2019_shrs_wide <- shrs_wide;      t53_2019_shrs_long <- shrs_long
t53_2019_benchmark <- bench_long;      t53_2019_grad_share <- grad_share
t53_2019_dominant_edu <- dominant_edu; t53_2019_diff_benchmark <- diff_from_bench
t53_2019_scorecard <- attain_scorecard; t53_2019_all_occ_grad <- all_occ_grad
```

```{r catalog, results='asis'}
cat("| Variable | Dims |\n|---|---|\n")
cat(paste0("| `t53_2019_shrs_wide` | ",nrow(shrs_wide),"x",ncol(shrs_wide)," |\n"))
cat(paste0("| `t53_2019_shrs_long` | ",nrow(shrs_long),"x",ncol(shrs_long)," |\n"))
cat(paste0("| `t53_2019_grad_share` | ",nrow(grad_share),"x",ncol(grad_share)," |\n"))
cat(paste0("| `t53_2019_scorecard` | ",nrow(attain_scorecard),"x",ncol(attain_scorecard)," |\n"))
cat("| `t53_2019_all_occ_grad` | scalar |\n")
```

**Source:** BLS Table 5.3, 2019 vintage. HIM (29-9021) not present.
