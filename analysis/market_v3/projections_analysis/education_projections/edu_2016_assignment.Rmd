---
title: "SHRS Education Analysis — 2016 Entry Requirements & Training (Assignment)"
...
CSV_PATH <- file.path(path.expand("~"), "Documents", "SHRS_Analysis",
                       ...
                       paste0("education_", VINTAGE, " - Assignment.csv"))
...
2016 uses merged "title and code" header.
...
```{r dq, results='asis'}
cat(paste0("**Vintage:** ",VINTAGE," (earliest) | **Programs:** ",nrow(shrs_t54)," of 9\n\n"))
...
shrs_t54 |>
  ...
  kable(caption=paste0("BLS Entry Requirements (",VINTAGE," — earliest vintage)"))|>
...
shrs_t54 <- shrs_t54 |>
  mutate(
    edu_tier=case_when(entry_education=="Doctoral or professional degree"~4,
                        entry_education=="Master's degree"~3,entry_education=="Bachelor's degree"~2,
...
shrs_t54 |>
  select(Program=shrs_program,`Entry Ed`=entry_education,Edu=edu_burden,Exp=exp_burden,
         OJT=ojt_burden,Total=total_burden,`Train`=d1_training_simple,
         `ExpAcc`=d2_exp_accessible,Composite=composite,Signal=signal) |>
  arrange(factor(Program,levels=soc_xwalk$shrs_program)) |>
...
shrs_t54|>select(shrs_program,edu_burden,exp_burden,ojt_burden)|>
  ...
  theme_minimal(base_size=13)+theme(axis.text.x=element_text(angle=30,hjust=1),legend.position="bottom")
```

```{r sc-rank, fig.height=5}
shrs_t54|>ggplot(aes(x=reorder(shrs_program,composite),y=composite,fill=signal))+
  geom_col(width=0.6)+geom_text(aes(label=composite),hjust=-0.1,size=4)+
  coord_flip()+scale_fill_manual(values=signal_colors)+scale_y_continuous(limits=c(0,110))+
  labs(title=paste0("Entry Composite (",VINTAGE," — earliest)"),x=NULL,y="Score")+
  theme_minimal(base_size=13)+theme(legend.position="bottom")
```

---

# 4 — Cross-Vintage Reclassification Summary

This is the earliest vintage. Key reclassifications across the full series:

```{r reclass-summary, results='asis'}
cat("| Program | 2016 Entry | 2021+ Entry | Change |\n")
cat("|---|---|---|---|\n")
cat("| AT | Bachelor's degree | Master's degree | Upgraded 2021 (CAATE mandate) |\n")
cat("| HIM | Not reported | Associate's degree | Added to BLS 2021 |\n")
cat("| All others | No change | No change | Stable across all vintages |\n")
```
...
```{r store}
t54_2016_data <- shrs_t54
t54_2016_scorecard <- shrs_t54 |>
  select(shrs_program,shrs_dept,soc_code,entry_education,work_experience,ojt_required,
         edu_tier,tier_label,edu_years,edu_burden,exp_burden,ojt_burden,total_burden,
         d1_training_simple,d2_exp_accessible,composite,signal)
```

```{r catalog, results='asis'}
cat("| Variable | Dims |\n|---|---|\n")
cat(paste0("| `t54_2016_data` | ",nrow(t54_2016_data),"x",ncol(t54_2016_data)," |\n"))
cat(paste0("| `t54_2016_scorecard` | ",nrow(t54_2016_scorecard),"x",ncol(t54_2016_scorecard)," |\n"))
```

**Source:** BLS Assignment table, 2016 vintage (earliest in series). HIM missing. AT = Bachelor's.
