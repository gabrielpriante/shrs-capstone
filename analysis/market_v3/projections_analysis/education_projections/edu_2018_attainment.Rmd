---
title: "SHRS Education Analysis — 2018 Workforce Attainment"
...
2018 uses merged "title and code" header with empty column 2.
...
shrs_long |>
  ...
  labs(title=paste0("Workforce Attainment (",VINTAGE,")"), subtitle="BLS Attainment Table — HIM not available",
       x=NULL, y="% of Workers") +
...
attain_scorecard|>
  ...
  labs(title=paste0("Attainment Composite (",VINTAGE,")"),x=NULL,y="Score")+
...
```{r store}
t53_2018_shrs_wide <- shrs_wide;      t53_2018_shrs_long <- shrs_long
t53_2018_benchmark <- bench_long;      t53_2018_grad_share <- grad_share
t53_2018_dominant_edu <- dominant_edu; t53_2018_diff_benchmark <- diff_from_bench
t53_2018_scorecard <- attain_scorecard; t53_2018_all_occ_grad <- all_occ_grad
```

```{r catalog, results='asis'}
cat("| Variable | Dims |\n|---|---|\n")
cat(paste0("| `t53_2018_shrs_wide` | ",nrow(shrs_wide),"x",ncol(shrs_wide)," |\n"))
cat(paste0("| `t53_2018_scorecard` | ",nrow(attain_scorecard),"x",ncol(attain_scorecard)," |\n"))
cat("| `t53_2018_all_occ_grad` | scalar |\n")
```

**Source:** BLS Attainment table, 2018 vintage. HIM not present.
