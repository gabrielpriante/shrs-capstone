---
title: "SHRS Education Analysis — 2021 Workforce Attainment (Table 5.3)"
...
shrs_long |>
  ...
  labs(title = paste0("Workforce Educational Attainment (", VINTAGE, ")"),
       subtitle = "BLS Table 5.3", x = NULL, y = "Percent (%)") +
...
shrs_wide |>
  select(Program = shrs_program, Dept = shrs_dept,
         `< HS` = less_than_hs, `HS` = hs_diploma, `Some Col` = some_college,
         `Assoc` = associates, `Bach` = bachelors, `Master` = masters,
         `Doct` = doctoral) |>
...
```{r store}
t53_2021_shrs_wide      <- shrs_wide
t53_2021_shrs_long      <- shrs_long
t53_2021_benchmark      <- bench_long
t53_2021_grad_share     <- grad_share
t53_2021_dominant_edu   <- dominant_edu
t53_2021_diff_benchmark <- diff_from_bench
t53_2021_scorecard      <- attain_scorecard
t53_2021_all_occ_grad   <- all_occ_grad
```

```{r catalog, results='asis'}
cat("| Variable | Description | Dims |\n|---|---|---|\n")
cat(paste0("| `t53_2021_shrs_wide` | Wide attainment | ", nrow(shrs_wide), " x ", ncol(shrs_wide), " |\n"))
cat(paste0("| `t53_2021_shrs_long` | Long attainment | ", nrow(shrs_long), " x ", ncol(shrs_long), " |\n"))
cat(paste0("| `t53_2021_grad_share` | Grad concentration | ", nrow(grad_share), " x ", ncol(grad_share), " |\n"))
cat(paste0("| `t53_2021_scorecard` | 3-dim scorecard | ", nrow(attain_scorecard), " x ", ncol(attain_scorecard), " |\n"))
cat("| `t53_2021_all_occ_grad` | Benchmark scalar | 1 |\n")
```

**Source:** BLS Table 5.3, 2021 vintage.
