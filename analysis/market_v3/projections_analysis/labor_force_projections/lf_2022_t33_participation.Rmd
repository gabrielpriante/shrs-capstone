---
title: "SHRS Market Analysis — 2022 Participation Rates (Table 3.3)"
...
```{r config}
VINTAGE <- "2022"
YR1 <- 2002; YR2 <- 2012; YR3 <- 2022; YR4 <- 2032
...
rate_trend |>
  ...
  labs(title=paste0("Labor Force Participation Rate Trend (",VINTAGE," Vintage)"),
...
tibble(
  Metric = c(paste0("LFPR ",YR1), paste0("LFPR ",YR2), paste0("LFPR ",YR3),
             ...
             paste0("Change ",YR1,"-",YR2), paste0("Change ",YR2,"-",YR3),
             paste0("Change ",YR3,"-",YR4)),
...
```{r race}
race <- raw |>
  filter(str_detect(group, "^White$|^White,|^Black$|^Black,|^Asian$|^Asian,|^Hispanic origin"))
...
# 6 — Cross-Vintage Comparison

```{r cross-vintage, results='asis'}
cat(paste0("### ",VINTAGE," Vintage Benchmarks\n\n"))
cat(paste0("- <span style='color:#2c7bb6;'>**LFPR ",YR3,":**</span> ", lfpr_current, "%\n"))
cat(paste0("- <span style='color:#2c7bb6;'>**LFPR ",YR4," (projected):**</span> ", lfpr_projected, "%\n"))
cat(paste0("- <span style='color:#2c7bb6;'>**Projected change:**</span> ", lfpr_change, " pp\n"))
```
...
```{r store}
lf_t33_2022_raw       <- raw
lf_t33_2022_total     <- total
lf_t33_2022_gender    <- gender
lf_t33_2022_age       <- if(exists("age")) age else NULL
lf_t33_2022_race      <- if(exists("race")) race else NULL
lf_t33_2022_benchmarks <- list(
...
cat(paste0("| `lf_t33_2022_raw` | Full T3.3 (",nrow(raw)," rows) |\n"))
cat("| `lf_t33_2022_total` | Total LFPR row |\n")
cat("| `lf_t33_2022_benchmarks` | Key benchmarks (list) |\n")
```

**Source:** BLS Table 3.3, 2022 vintage. Projection: 2022–2032.
