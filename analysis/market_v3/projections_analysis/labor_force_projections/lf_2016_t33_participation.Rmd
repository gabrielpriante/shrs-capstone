---
title: "SHRS Market Analysis — 2016 Participation Rates (Table 3.3)"
...
CSV_PATH <- file.path(path.expand("~"), "Documents", "SHRS_Analysis",
                       ...
                       "2016_labor-force - Table 3.3.csv")
```

# 1 — Load & Clean

```{r load}
raw <- read_csv(CSV_PATH, skip = 4, col_names = FALSE, show_col_types = FALSE)
...
```{r store}
lf_t33_2016_raw <- raw
lf_t33_2016_total <- total
lf_t33_2016_benchmarks <- list(lfpr_current=lfpr_current, lfpr_projected=lfpr_projected,
                                     lfpr_change_pp=lfpr_change, yr3=YR3, yr4=YR4)
```

```{r catalog, results='asis'}
cat("| Variable | Description |\n|---|---|\n")
cat(paste0("| `lf_t33_2016_raw` | Full data (",nrow(raw)," rows) |\n"))
cat("| `lf_t33_2016_benchmarks` | Key benchmarks (list) |\n")
```

**Source:** BLS Table 3.3, 2016 vintage (2016-2026).
